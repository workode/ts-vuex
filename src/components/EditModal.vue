  ...
  setup(props: any) {
    ...
    const setFields = () => {
      ...
      if(task) {
        ...
        state.deliveryDate = task.deliveryDate
        state.tags = task.tags
        state.closeToDeliveryDate = task.closeToDeliveryDate
      }
    }
    onMounted(() => { setFields() });
    const updateTask = () => {
      if (
          ...
          state.deliveryDate === '' ||
          state.tags === '' ||
          state.closeToDeliveryDate === false
      )
        return
      const task: TaskItem = {
        id: props.id,
        ...
        tags: state.tags,
        closeToDeliveryDate: state.closeToDeliveryDate
      }
